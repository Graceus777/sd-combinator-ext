...
def load_lora_characters(lora_dir: str) -> dict:
    """Load character LoRAs from lora_texts/ directory.

    Scans for *.json files at the top level (skips subdirectories like nsfw_scenes/).
    Returns dict mapping stem name -> {"lora": stem, "activation": ..., "weight": ...}.
    """
    chars = {}
    ...
    for fname in sorted(os.listdir(lora_dir)):
        ...
        if not activation:
            continue  # skip files with no activation text

        weight = data.get("preferred weight", 0.8)
        if weight == 0:
            weight = 0.8  # default when not set
