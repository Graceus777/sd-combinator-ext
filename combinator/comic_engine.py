...
def panel_already_generated(output_dir: str, title_tag: str, panel_id: str) -> bool:
    ...
    pattern = os.path.join(output_dir, f"*_{title_tag}_{panel_id}_*.png")
...
def generate_panel_candidates(
    ...
    negative = panel.get("negative_extra", "")
    if negative and base_negative:
        negative = f"{negative}, {base_negative}"
    elif base_negative:
        negative = base_negative
    if character is None:
        negative = (f"1girl, solo, person, human, figure, {negative}" if negative
                    else f"1girl, solo, person, human, figure, {base_negative}")
    ...
    do_adetailer = False if character is None else gen.get("adetailer", True)
...
def generate_panel(
    ...
    if effective_candidates > 1:
        ok, files = generate_panel_candidates(
            ...
            init_image_path=init_image_path, log=log,
        )
...
def preview_script(script: Dict[str, Any], source_label: str = "") -> str:
    ...
    if cam_warnings:
        lines.append("")
        lines.append("**Camera continuity warnings:**")
...
def run_comic_script(
    ...
    for idx, (pi, qi, panel, page) in enumerate(panels):
        ...
        ok = generate_panel(
            ...
            generated_paths=generated_paths, log=log,
        )
        if ok:
            generated += 1
